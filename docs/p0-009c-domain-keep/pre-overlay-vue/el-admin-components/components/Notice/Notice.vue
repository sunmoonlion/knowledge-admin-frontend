  ...
  <div>
    <el-dropdown trigger="click">
      <span class="el-dropdown-link">
        <Notification v-bind="notificationProps"></Notification>
      </span>
      <template #dropdown>
        <Lists
          :lists="lists"
...
const notificationProps = computed(() => {
  const base = { icon: 'ep:bell', size: 12, scale: 1 }
  const { icon, size, scale, ...rest } = (filterdProps.value || {}) as Record<string, any>
  return {
    ...rest,
    icon: icon ?? base.icon,
    size: size ?? base.size,
